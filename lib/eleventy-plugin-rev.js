...
const createRevvedOutputFilter = function(currentPage) {
  ...
  return function(inputPath) {
    ...
    if (!settings.isEnabled(path.extname(inputPath))) {
      if (pluginOutputPath) {
        if (outputPathFromInputPath === undefined)
    ...
    if (!outputPath) {
      logger.error(`\`revvedOutput\` filter failed to find an outputPath for ${ inputPath } when processing ${ currentPage(this).inputPath }.`);
      return "";
    }
    let hash = revHashFromInputPath(normalizedInputPath);
    if (hash === undefined) {
      logger.error(`\`revvedOutput\` filter failed to find a revision hash for ${ inputPath } when processing ${ currentPage(this).inputPath }.`);
      return "";
    }
    return revvedFilePath(outputPath, hash);
  };
