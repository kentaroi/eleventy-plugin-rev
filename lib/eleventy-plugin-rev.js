...
const createRevvedOutputFilter = function(currentPage) {
  return function(inputPath) {
    ...
    if (!settings.isEnabled(path.extname(inputPath))) {
      logger.error(`\`revvedOutput\` filter is called with ${ inputPath } when processing ${ currentPage(this).inputPath }, although the plugin is disabled.`);
      // TODO try to use elventy-plugin-output-path if available
      return new Error(`\`revvedOutput\` filter was called from ${ currentPage(this).inputPath }, while eleventy-plugin-rev was disabled.`);
    }
